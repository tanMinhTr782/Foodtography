...
import { View, Text, Image, TouchableHighlight, TextInput, TouchableOpacity, Alert } from 'react-native';
...
export const EditProfile = () => {
    ...
    const handleSetUser = () => {
        const newUserInfo = user.replace(
            user.substring(user.indexOf('name') + 7, user.indexOf(',', user.indexOf('name') + 7) - 1),
            name,
        );
        const newUserInfo2 = newUserInfo.replace(
            newUserInfo.substring(
                newUserInfo.indexOf('avatar') + 9,
                newUserInfo.indexOf(',', newUserInfo.indexOf('avatar') + 9) - 1,
            ),
            imageUrl,
        );
    ...
    const showAlert = () =>
        Alert.alert('Alert', "You haven't saved your changes yet, are you sure you want to do that?", [
            {
                text: 'OK',
    ...
    const showSuccessAlert = () =>
        Alert.alert('Alert', 'Update information successfully!!', [
            {
                text: 'OK',
                style: 'cancel',
            },
        ...
        <>
            <View style={styles.createRecipeContainer}>
                <View style={{ flexDirection: 'row' }}>
                    <TouchableOpacity
                        onPress={() => {
                            if (saveChange) navigation.goBack();
                            else showAlert();
                        }}
                ...
                <View style={styles.firstComponentContainers}>
                    ...
                    <Text style={styles.componentArrow}>{email}</Text>
                </View>
